...
class QuizQuestionFeedbackForm extends FormBase {
  ...
  function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($quiz_result->getLayout() as $question) {
      if ($question->get('number')->getString() == $question_number && $question->qqr_pid) {
    ...
    if (empty($out)) {
      ...
      $out[] = array(
        '#title' => t('Question @num', array('@num' => $quiz_result->getLayout()[$question_number]->get('display_number')->getString())),
        '#type' => 'fieldset',
        'feedback' => $feedback,
      );
    }
  ...
  function submitEnd($form, &$form_state) {
    ...
    $quiz = $form_state->getBuildInfo()['args'][0];
    $form_state->setRedirect('entity.quiz_result.canonical', ['quiz' => $quiz->id(), 'quiz_result' => $result_id]);
  }
