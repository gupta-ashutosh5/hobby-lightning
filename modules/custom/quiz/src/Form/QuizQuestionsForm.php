...
class QuizQuestionsForm extends FormBase {
  ...
  function _quiz_add_fields_for_creating_questions(&$form, &$types, Quiz $quiz) {
    ...
    foreach ($types as $type => $info) {
      ...
      $form['additional_questions'][$type] = array(
        '#markup' => '<div class="add-questions">' . Link::fromTextAndUrl($info['label'], $url)->toString() . '</div>',
        '#access' => $access,
      );
    }
    if (!$create_question) {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $header = array_merge($header, ['Revision', 'Operations', 'Weight', 'Parent']);

    // Display questions in this quiz.
    $form['question_list'] = [
      '#type' => 'table',
      '#title' => t('Questions in this @quiz', array('@quiz' => _quiz_get_quiz_name())),
      '#type' => 'table',
      '#header' => $header,
      '#empty' => t('There are currently no questions in this @quiz. Assign existing questions by using the question browser below. You can also use the links above to create new questions.', array('@quiz' => _quiz_get_quiz_name())),
      '#tabledrag' => [
        [
          'action' => 'match',
          'relationship' => 'parent',
    ...
    // same time.
    $form['timestamp'] = array('#type' => 'hidden', '#default_value' => \Drupal::time()->getRequestTime());
  ...
  function _quiz_add_revision_checkbox(&$form, $quiz) {
    ...
    if ($quiz->hasAttempts()) {
      $results_url = Url::fromRoute('view.quiz_results.list', ['quiz' => $quiz->id()])->toString();
      $quiz_url = Url::fromRoute('entity.quiz.edit_form', ['quiz' => $quiz->id()], ['query' => \Drupal::destination()->getAsArray()])->toString();
      $form['revision_help'] = [
        '#markup' => t('This quiz has been answered. To make changes to the quiz you must either <a href="@results_url">delete all results</a> or <a href="@quiz_url">create a new revision</a>.', ['@results_url' => $results_url, '@quiz_url' => $quiz_url]),
      ];
  ...
  function _quiz_add_questions_to_form(&$form, &$questions, &$quiz, &$question_types) {
    foreach ($questions as $id => $question_relationship) {
      ...
      if ($quiz_question->access('view')) {
        $question_titles = ['#markup' => Link::fromTextAndUrl($quiz_question->get('title')->getString(), $view_url)->toString()];
      }
      else {
        $question_titles = ['#plain_text' => $quiz_question->get('title')->getString()];
      }
      ...
      $table[$id]['auto_update_max_score'] = array(
        ...
        '#default_value' => $question_relationship->get('auto_update_max_score')->getString() ? $question_relationship->get('auto_update_max_score')->getString() : 0,
      );
      ...
      if ($question_relationship->get('question_vid')->value == $latest_quiz_question->getRevisionId()) {
        $update_cell = array(
      ...
      $table[$id]['#weight'] = (int) $question_relationship->get('weight')->getString();
      $table[$id]['weight'] = array(
        ...
        '#type' => 'number',
        '#default_value' => (int) $question_relationship->get('weight')->getString(),
        '#attributes' => [
          'class' => [
            'table-sort-weight',
          ],
